...
<?php
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_row()) {
			...
			echo "<tr><td>".$row[0]."</td><td>".$row[1]."</td><td>".($row[2]-$quantity[$x])."</td><td>$".$row[3]."</td><td><input type=\"number\" name=\"quantity".$x."\" id=\"quantity".$x."\" min=\"0\" max=\"".($max_stock_row[0]-$quantity[$x])."\" value=\"0\"></td></tr>";
			$x++;
		}
	} else {
...
<input type="submit" name="submit" value = "Put into cart!" />
</form>
